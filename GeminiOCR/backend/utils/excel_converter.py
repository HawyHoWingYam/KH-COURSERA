...
def extract_text_value(value: Any) -> Any:
    ...
    if isinstance(value, dict) and "text" in value and isinstance(value["text"], (str, int, float)):
        return value["text"]
    return value
...
def json_to_excel(json_datas: Union[Dict, List], output_path: str, doc_type_code: str = None) -> str:
    ...
    with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
        ...
        if main_df.empty:
            ...
            main_df = pd.DataFrame({"No Data": ["No structured data found in the input JSON"]})
        ...
        for idx, col in enumerate(main_df.columns):
            ...
            worksheet.column_dimensions[col_letter].width = min(max_len, 50)  # Cap at 50
