...
class Config:
    ...
    def _load_config(self):
        ...
        # Database
        self.database_url = os.getenv("DATABASE_URL") or self._get_from_config(
            "database_url"
        )
        ...
        self.aws_access_key_id = os.getenv(
            "AWS_ACCESS_KEY_ID"
        ) or self._get_from_config("aws_access_key_id")
        self.aws_secret_access_key = os.getenv(
            "AWS_SECRET_ACCESS_KEY"
        ) or self._get_from_config("aws_secret_access_key")
        self.aws_default_region = os.getenv("AWS_DEFAULT_REGION", "ap-southeast-1")
        self.aws_secret_name = os.getenv("AWS_SECRET_NAME") or self._get_from_config(
            "aws_secret_name"
        )
